...
# Финальная версия v2: с прокруткой модального окна

MAX_SELLERS="${1:-3}"
MODAL_SCROLLS="${2:-5}"  # Количество прокруток модального окна

echo "🚀 Сбор продавцов Ozon Global (v2 с прокруткой модального окна)"
echo "Цель: $MAX_SELLERS продавцов"
echo "Прокруток модального окна: $MODAL_SCROLLS"
echo ""
...
"' >/dev/null 2>&1
                ...
                echo "    📜 Прокручиваю модальное окно..."

                # Прокручиваем модальное окно несколько раз
                for ((scroll=1; scroll<=$MODAL_SCROLLS; scroll++)); do
                    echo -ne "\r    📜 Прокрутка модального окна: $scroll/$MODAL_SCROLLS    "
...
if (modal) {
    // Прокручиваем вниз
    modal.scrollTop = modal.scrollTop + modal.clientHeight;
}
"' >/dev/null 2>&1
...
result = {
    ...
    'filter': 'delivery > 21 days + cheaper option with scroll',
    'modal_scrolls': $MODAL_SCROLLS,
    'sellers': [{'url': url} for url in sellers]
}

timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
filename = f'results/sellers_final_v2_{timestamp}.json'
